import{_ as n,c as s,o as a,b as t}from"./app-B5uJTUF5.js";const e="/assets/image-BgWh_IE7.png",p="/assets/image2-BcqzzwnX.png",o={},c=t(`<h1 id="使用workflows实现对vuepress的一键部署" tabindex="-1"><a class="header-anchor" href="#使用workflows实现对vuepress的一键部署"><span>使用workflows实现对vuepress的一键部署</span></a></h1><p><strong>服务端配置记录</strong></p><p><strong>创建git用户</strong></p><pre><code class="language-bash"><span class="token comment"># 创建一个用户</span>
...
</code></pre><p><strong>为github的主机生成私钥和公钥</strong></p><pre><code class="language-bash"><span class="token comment"># 为github的主机生成私钥</span>
...
</code></pre><p><strong>在github的仓库创建私钥变量</strong></p><p>变量 名为 <code>SSH_PRIVATE_KEY</code> 值为 刚刚生成的私钥内容</p><p><img src="`+e+`" alt="Alt text"></p><p><strong>编写workflows配置文件</strong></p><blockquote><ul><li>此时在该配置文件中就能通过，<code>\${{ secrets.SSH_PRIVATE_KEY }}</code> 获取到配置的私钥地址，</li><li>然后由于已经将其公钥添加到了自己的服务器中，所以github的服务器就能和自己的服务器建立免密链接</li></ul></blockquote><pre><code class="language-yml"><span class="token key atrule">name</span><span class="token punctuation">:</span> Deploy blog
...
</code></pre><p><strong>将本地分支提交到github仓库</strong></p><pre><code class="language-bash"><span class="token comment"># 添加远程地址</span>
