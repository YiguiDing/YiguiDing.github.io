...
</code></pre><p><strong>3.更新</strong></p><pre><code class="language-bash">apk update
</code></pre><p><strong>4.另一种一步到位的方法</strong></p><pre><code class="language-bash"><span class="token comment"># 配置</span>
...
apk update
</code></pre><h2 id="二、安装软件" tabindex="-1"><a class="header-anchor" href="#二、安装软件"><span>二、安装软件</span></a></h2><pre><code class="language-bash">apk <span class="token function">add</span> openrc
...
</code></pre><h2 id="三、配置服务管理openrc" tabindex="-1"><a class="header-anchor" href="#三、配置服务管理openrc"><span>三、配置服务管理openrc</span></a></h2><p><strong>1.安装openrc</strong></p><pre><code class="language-bash">apk <span class="token function">add</span> openrc
<span class="token comment"># 注意 要先安装openrc 再安装要启动的服务比如docker 否则提示服务不存在</span>
</code></pre><p><strong>2.编辑/etc/inittab 将<code>::sysinit:/sbin/openrc sysinit</code>中的<code>sysinit</code>删除</strong></p><pre><code class="language-bash"><span class="token function">vi</span> /etc/inittab
</code></pre><p><strong>3.重启ish</strong></p><p><strong>4.添加服务</strong></p><pre><code class="language-bash">rc-update <span class="token function">add</span> sshd
</code></pre><p><strong>5.删除服务</strong></p><pre><code class="language-bash">rc-update del sshd
</code></pre><p><strong>6.启动服务</strong></p><pre><code class="language-bash">rc-service sshd start
</code></pre><p><strong>7.暂停服务</strong></p><pre><code class="language-bash">rc-service sshd stop
</code></pre><p><strong>8.查询服务状态</strong></p><pre><code class="language-bash">rc-status <span class="token parameter variable">-a</span>
</code></pre><h2 id="配置开启ssh服务" tabindex="-1"><a class="header-anchor" href="#配置开启ssh服务"><span>配置开启ssh服务</span></a></h2><p><strong>1. 安装ssh</strong></p><pre><code class="language-bash">apk <span class="token function">add</span> openssh
</code></pre><p><strong>2.为本机生成SSH主机秘钥</strong></p><pre><code class="language-bash">ssh-keygen <span class="token parameter variable">-A</span>
</code></pre><p><strong>3.临时启动守护进程</strong></p><pre><code class="language-bash">/usr/sbin/sshd
</code></pre><p><strong>4.将守护进程添加到开机启动</strong></p><pre><code class="language-bash">rc-update <span class="token function">add</span> sshd
<span class="token comment"># 移除开机自启服务：rc-update del sshd</span>
</code></pre><p><strong>5.启动服务</strong></p><pre><code class="language-bash">rc-service sshd start
<span class="token comment"># 暂停服务：rc-service sshd stop</span>
</code></pre><p><strong>6.配置sshd</strong></p><pre><code class="language-bash"><span class="token function">cat</span> <span class="token operator">&gt;&gt;</span> /etc/ssh/sshd_config  <span class="token operator">&lt;&lt;</span> <span class="token string">EOF
# 允许root登录 禁止密码登录
...
EOF</span>
</code></pre><h2 id="实现ish后台运行配置" tabindex="-1"><a class="header-anchor" href="#实现ish后台运行配置"><span>实现ish后台运行配置</span></a></h2><p><strong>1.关键代码</strong></p><pre><code class="language-bash"><span class="token comment"># 说明：ish是ios系统上的应用程序，为使iSH进程能够支持长期后台运行而不被杀死，需要不断获取手机位置信息</span>
<span class="token function">cat</span> /dev/location <span class="token operator">&gt;</span> /dev/null <span class="token operator">&amp;</span>
</code></pre><p><strong>2.配置</strong></p><pre><code class="language-bash"><span class="token builtin class-name">echo</span> <span class="token string">&#39;cat /dev/location &gt; /dev/null &amp;&#39;</span> <span class="token operator">&gt;&gt;</span> .profile
<span class="token function">cat</span> .profile <span class="token comment"># 检查配置是否成功</span>
</code></pre><h2 id="尝试安装vscode网页版" tabindex="-1"><a class="header-anchor" href="#尝试安装vscode网页版"><span>尝试安装vsCode网页版</span></a></h2><h3 id="尝试1-在ish中尝试通过npm安装-配置失败" tabindex="-1"><a class="header-anchor" href="#尝试1-在ish中尝试通过npm安装-配置失败"><span>尝试1：在iSH中尝试通过npm安装（配置失败）</span></a></h3><p><strong>先要安装<code>npm</code>和<code>nodejs</code></strong><br> 但由于npm软件最新版使用了一些ish没有实现的指令 导致运行npm会提示非法指令 所以只能安装低版本</p><p><strong>1.修改 /etc/apk/repositories 将版本改为3.12版本</strong></p><pre><code class="language-bash"><span class="token comment"># 修改</span>
<span class="token function">cat</span> <span class="token operator">&gt;&gt;</span> .profile <span class="token operator">&lt;&lt;</span> <span class="token string">EOF
...
</code></pre><h3 id="尝试2-在ish中尝试通过docker安装-配置失败" tabindex="-1"><a class="header-anchor" href="#尝试2-在ish中尝试通过docker安装-配置失败"><span>尝试2：在iSH中尝试通过docker安装（配置失败）</span></a></h3><p><strong>1.安装docker和compose</strong></p><pre><code class="language-bash">apk <span class="token function">add</span> <span class="token function">docker</span>
...
</code></pre><h3 id="尝试3-在utm中安装alpine3-16-1再尝试使用npm安装-配置失败" tabindex="-1"><a class="header-anchor" href="#尝试3-在utm中安装alpine3-16-1再尝试使用npm安装-配置失败"><span>尝试3：在UTM中安装Alpine3.16.1再尝试使用npm安装（配置失败）</span></a></h3><p>安装UTM 在UTM中安装Alpine3.16.1</p><pre><code class="language-bash"><span class="token comment"># 安装时最好设置好密码</span>
...
</code></pre><p>尝试1、3出错原因和这个类似：<a href="https://github.com/coder/code-server/issues/5184" target="_blank" rel="noopener noreferrer">#5184</a></p><h3 id="尝试4-在utm中安装alpine3-16-1再尝试使用yarn安装-配置失败" tabindex="-1"><a class="header-anchor" href="#尝试4-在utm中安装alpine3-16-1再尝试使用yarn安装-配置失败"><span>尝试4：在UTM中安装Alpine3.16.1再尝试使用yarn安装（配置失败）</span></a></h3><p>该方法根据自issue：<a href="https://github.com/coder/code-server/issues/5174" target="_blank" rel="noopener noreferrer">#5174</a></p><pre><code class="language-bash"><span class="token function">npm</span> <span class="token function">install</span> <span class="token parameter variable">-g</span> <span class="token function">yarn</span>
...
</code></pre><h3 id="尝试5-在utm中安装alpine3-16-1再尝试使用release版本安装-配置失败" tabindex="-1"><a class="header-anchor" href="#尝试5-在utm中安装alpine3-16-1再尝试使用release版本安装-配置失败"><span>尝试5：在UTM中安装Alpine3.16.1再尝试使用release版本安装（配置失败）</span></a></h3><p>既然在UTM安装的是64bit版本的系统 为什么不直接使用release版本？？？</p><pre><code class="language-bash"><span class="token comment"># 下载</span>
...
</code></pre><h3 id="尝试6-在utm中安装alpine3-16-1并开启jit再尝试使用release版本安装-成功" tabindex="-1"><a class="header-anchor" href="#尝试6-在utm中安装alpine3-16-1并开启jit再尝试使用release版本安装-成功"><span>尝试6：在UTM中安装Alpine3.16.1并开启JIT再尝试使用release版本安装（成功）</span></a></h3><p>方法同上，UTM使用JIT（jitterbug）启动，终于可以了！！！原来是运行速度的问题</p><p>完整过程：</p><pre><code class="language-bash"><span class="token comment"># 下载</span>
<span class="token function">wget</span> https://github.com/coder/code-server/releases/download/v4.5.1/code-server-4.5.1-linux-amd64.tar.gz
<span class="token comment"># 解压</span>
<span class="token function">tar</span> <span class="token parameter variable">-xvf</span> ./code-server-4.5.1-linux-amd64
