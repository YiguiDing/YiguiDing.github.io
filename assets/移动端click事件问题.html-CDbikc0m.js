import{_ as n,c as s,o as a,b as t}from"./app-BSX0n7Vp.js";const p={},o=t(`<h1 id="移动端click事件问题" tabindex="-1"><a class="header-anchor" href="#移动端click事件问题"><span>移动端click事件问题</span></a></h1><p>移动端click事件会有300ms的延迟，因为最初移动端需要用两只手指来放大页面，然后通过双击页面来恢复页面大小（double tap to zoom），300ms的延迟就是用来判断是否双击屏幕的，若两次点击屏幕的间隔小于300ms则判断是双击屏幕，大于300ms则被判断为click事件</p><h2 id="解决方案-禁用缩放" tabindex="-1"><a class="header-anchor" href="#解决方案-禁用缩放"><span>解决方案:禁用缩放</span></a></h2><p>禁用缩放后也就不存在双击屏幕恢复页面默认大小的功能，于是300ms的延迟就不存在了</p><pre><code class="language-html"><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>meta</span> <span class="token attr-name">name</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&quot;</span>viewport<span class="token punctuation">&quot;</span></span> <span class="token attr-name">content</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&quot;</span>user-scalable=no<span class="token punctuation">&quot;</span></span><span class="token punctuation">&gt;</span></span>
</code></pre><h2 id="解决方案-利用touch事件自己封装函数" tabindex="-1"><a class="header-anchor" href="#解决方案-利用touch事件自己封装函数"><span>解决方案:利用touch事件自己封装函数</span></a></h2><ul><li>手指触摸时记录触摸时间</li><li>手指离开时记录离开时间</li><li>手指移动时记录是否移动</li><li>当手指触摸和离开时间间隔小于150ms 且手指没有滑动屏幕，则判断为点击</li></ul><pre><code class="language-js"><span class="token keyword">function</span> <span class="token function">tap</span><span class="token punctuation">(</span><span class="token parameter">obj<span class="token punctuation">,</span>callback</span><span class="token punctuation">)</span><span class="token punctuation">{</span>
...
</code></pre><h2 id="解决方案-使用fastclick-js插件" tabindex="-1"><a class="header-anchor" href="#解决方案-使用fastclick-js插件"><span>解决方案:使用fastclick.js插件</span></a></h2><p>github仓库：<a href="https://github.com/ftlabs/fastclick" target="_blank" rel="noopener noreferrer">fastclick.js</a></p><p>使用：****</p><pre><code class="language-html"><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;</span>script</span> <span class="token attr-name">type</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&#39;</span>application/javascript<span class="token punctuation">&#39;</span></span> <span class="token attr-name">src</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">&#39;</span>/path/to/fastclick.js<span class="token punctuation">&#39;</span></span><span class="token punctuation">&gt;</span></span><span class="token script"></span><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>script</span><span class="token punctuation">&gt;</span></span>
...
</code></pre><p><strong>注意</strong></p><p>Note: As of late 2015 most mobile browsers - notably Chrome and Safari - no longer have a 300ms touch delay, so fastclick offers no benefit on newer browsers, and risks introducing bugs into your application. Consider carefully whether you really need to use it.</p><p>截止到2015年，Chrome and Safari已经不再有300毫秒的触摸延迟了</p><p><strong>完整源代码</strong></p><pre><code class="language-js"><span class="token punctuation">;</span><span class="token punctuation">(</span><span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
