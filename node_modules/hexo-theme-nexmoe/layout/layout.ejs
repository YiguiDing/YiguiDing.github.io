...
<head>
    ...
    <!-- QQ内置分享会从这里读取分享的标题，图片，描述 -->
                <meta itemprop="name" content="<% if (title){ %><%= title %> - <% } %><%= config.title %>"/>
                <meta id="share_image_address" itemprop="image" content="图片URL" />
                <!-- <meta name="description" itemprop="description" content="描述" /> -->
                <script>
                    var encodePageURI = encodeURI("<% if (page.cover){ %><%= page.cover %><% }else{ %><%= theme.favicon.href %><% } %>")
                    timer = null
                    timer=setInterval(function(){
                        do
                        {
                            document.getElementById("share_image_address").content=encodePageURI
                        }while(document.getElementById("share_image_address").content!=encodePageURI)
                        clearInterval(timer)
                    },
                    100
                    )
    ...
    <script  defer src="<%- theme.cdn.jquery.js %>"></script>
    
    <script  defer src="<%- theme.cdn.lazysizes.js %>"></script>
    <script defer src="<%- theme.cdn.mdui.js %>"></script>

    <% if (theme.iconlib) { %>
        <link rel="stylesheet" href="<%= theme.iconlib %>">
    <% } %>
    <%- css_auto_version('lib/mdui_043tiny/css/mdui.css') %>
    <!-- 网站css图标 -->
    <%- css_auto_version('lib/iconfont/iconfont.css') %>
    <!-- 社交css图标 -->
    <link rel="stylesheet" href="/lib/social_logo88_icons/iconfont.css">
    <link rel="stylesheet" href="/lib/font_ti9q4a8wfrh/iconfont.css">
    <%- css_auto_version('css/style') %>
    <% if (theme.function.darkmode) { %>
        <%- css_auto_version('css/dark') %>
    <% } %> 
