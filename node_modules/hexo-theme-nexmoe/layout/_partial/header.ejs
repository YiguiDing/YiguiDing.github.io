<div class="nexmoe-drawer mdui-drawer" id="drawer">
    ...
    <div class="author-name">
        <a href="<%- url_for() %>" title="<%= config.author || config.title %>">
            <h1><%= config.author %></h1>
            <div class="author_motto"><%= config.motto %></div>
        </a>
    </div>
    <div class="nexmoe-count">
        <div><span><%= __('count.articles') %></span><%= site.posts.length %></div>
        <div><span><%= __('count.totalWord') %></span><%= totalcount(site) %></div>
    ...
    <div class="nexmoe-list mdui-list" mdui-collapse="{accordion: true}">
        ...
        <a class="nexmoe-list-item mdui-list-item mdui-ripple <%= is_current(theme.menu[name][0]) %>" href="<%- url_for(theme.menu[name][0]) -%>" title="<%= name %>">
            <i class="mdui-list-item-icon iconfont nexmoefont <% if (theme.menu[name][1]){ %><%= theme.menu[name][1] %><%} else{ %><%= 'icon-unorderedlist' %><% } %>"></i>
