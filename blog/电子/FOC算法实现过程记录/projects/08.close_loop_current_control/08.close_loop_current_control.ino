...
CurrentSensor currentSensor = CurrentSensor(
  ...
  []() {
    // [0,16383] => [0,5]
    static const float k = 5.0f / 16383.0f / 0.01f / 50.0f;
    // static const float k = 5.0f / 16383.0f;
    // Serial.print("raw_ia:");
    // Serial.print(k * analogRead(M1_Ia));
    // Serial.print(',');
    // Serial.print("raw_ib:");
    // Serial.print(k * analogRead(M1_Ib));
    // Serial.print(',');
    return CurrentABC{
      // i = u/r
      .a = k * analogRead(M1_Ia),
      .b = -k * analogRead(M1_Ib),  // b电路接反了 加符号
    };
  });
