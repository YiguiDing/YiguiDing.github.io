...
void Command::onSetTarget(uint8_t ch, float target)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (this->motor->controlMode != BLDCMotor::ControlMode::VelocityCurrent)
            this->motor->setMode(BLDCMotor::ControlMode::VelocityCurrent);
        this->motor->setTarget(target);
        // this->motor->pid_velocity_controller.reset();
        break;
    case 3:
        if (this->motor->controlMode != BLDCMotor::ControlMode::PositionCurrent)
            this->motor->setMode(BLDCMotor::ControlMode::PositionCurrent);
        this->motor->setTarget(target);
        // this->motor->pid_position_controller.reset();
        break;
    case 4:
        if (this->motor->controlMode != BLDCMotor::ControlMode::PositionVelocityCurrent)
            this->motor->setMode(BLDCMotor::ControlMode::PositionVelocityCurrent);
        this->motor->setTarget(target);
        break;
        // this->motor->pid_position_controller.reset();
    default:
