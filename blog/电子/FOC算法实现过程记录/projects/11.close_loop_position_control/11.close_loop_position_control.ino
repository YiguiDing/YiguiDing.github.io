...
void loop()
{
  ...
  // #include "PLL.cpp"
  // PLL pll;
  // float theta_rad = sensor.getPositon() / (float)UINT16_MAX * 2 * PI;
  // float theta_rad_hat = pll.update(theta_rad);

  // float velocity_radps = sensor.getVelocity();
  // float velocity_radps_hat = pll.getVelocity();

  // static uint8_t idx = 0;
  // if (++idx % 64 == 0)
  // {
  //   idx = 0;
  //   command.drawDragram(4, theta_rad, theta_rad_hat);
  //   command.drawDragram(5, velocity_radps, velocity_radps_hat);
  // }
  // command.update();
}