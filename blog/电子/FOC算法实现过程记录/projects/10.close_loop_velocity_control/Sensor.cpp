...
{
    // offset
    // this->motor->setPhraseVoltage(0.1f * INT16_MAX, 0, 0);        // 0°位置 d轴
    this->motor->setPhraseVoltage(0, 0.2f * INT16_MAX, _3_PI_2_); // -90°位置q轴施加电压，或0度d轴施加电压
    delay(2000);
...
{
    return this->directron * (this->full_rotations * _TWO_PI_ + this->angle / (float)UINT16_MAX * _TWO_PI_);
}
/**
 * 获取速度
 * 单位: rad/s
