...
\begin{align*}
    \text{Var}(\mathbf{e_k})
        &= E[e_ke_k^T] \\
        &= E[[(I - K_kH)\mathbf{e^-_k} - K_k\mathbf{v}_k ][(I - K_kH)\mathbf{e^-_k} - K_k\mathbf{v}_k ]^T] & \text{(公式:$(AB)^T=B^TA^T;(A+B)^T=A^T+B^T$)}\\
        &= E[[(I - K_kH)\mathbf{e^-_k} - K_k\mathbf{v}_k ][\mathbf{e^-_k}^T(I - K_kH)^T - \mathbf{v}_k^TK_k^T ]] & \text{(公式)} \\
...
d\frac{\text{tr}(ABA^T)}{dA}=d\frac{\text{tr}(A^TAB)}{d(A^TA)} \times \frac{d(A^TA)}{d(A)} = B \times (A+A^T) =
...
\begin{align*}
    ...
    d\frac{\text{tr}(\text{第C项})}{dK_k} &= d\frac{\text{tr}(K_kHP^-_k)}{dK_k} = (HP^-_k)^T = {P^-_k}^TH^T = P^-_kH^T \\
    d\frac{\text{tr}(\text{第D项})}{dK_k} &= d\frac{\text{tr}(K_kHP^-_kH^TK_k^T)}{dK_k} = (HP^-_kH^TK_k^T)^T = \\
...
\begin{align*}
    d\frac{\text{tr}(\text{Var}(\mathbf{e_k}))}{dK_k} &= 0 \\
    d\frac{\text{tr}(P^-_k) - \text{tr}(K_kHP^-_k) + \text{tr}(K_kHP^-_kH^TK_k^T) + \text{tr}(K_kRK_k^T)}{dK_k} &= 0 \\
    0 + 
\end{align*}
$$