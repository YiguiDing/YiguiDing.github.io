...
- 电路分析
  ...
  - 根据$t=t_{ON}$时$i=I_2$以及$t=t_{ON}+t_{off}$时$i=I_1$,联立两个表达式，可以求解出$I_1$和$I_2$，从而求解出整个电路的稳态解。
- 思考
...
- 工程师视角
  ...
  - 根据能量守恒，电路进入稳态后，$T_{on}$周期电感吸收的功率必然等于$T_{off}$周期电感发出的功率。
  - 从而得到输入输出和占空比的关系：$U_{out}=U_s\frac{t_{on}}{T}$
