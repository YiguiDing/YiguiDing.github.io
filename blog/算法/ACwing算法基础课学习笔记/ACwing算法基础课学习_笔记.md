...
- 约数之和： $(p_1^0 + p_1^1 + ... + p_1^{c_1}) *(p_2^0 + p_2^1 + ... + p_2^{c_2})*...* (p_k^0 + p_k^1 + ... + p_k^{c_k})$
  ...
  - $(...)+(...)+(...)+...+(...)$
  - $(p_1^{0}*p_2^{0}*p_3^{0}*...*p_k^{0})+(...)+...+(\text{括号中的乘积实际就是一个约数d})$
    - 质数N的任何一个约数都可以写成 $d=p_1^{β_1}*p_2^{β_2}*...*p_k^{β_k}  (β_k∈[0,c_k])$
...
- 通过费马定理，可知：
- 如果b、m互质，**且m是质数的话**，即:$gcd(b,m)=1$ 且 $m=\text{质数}p$
- 则有：$b^{p-1}≡1 (\mod p)$
...
- 设$M=m_1*m_2*...*m_k$
- 设$M_i=\frac{M}{m_i}$
- 设 $M_i^{-1}$ 为 $M_i$ 模 $m_i$ 的逆，即 $M_i*M_i^{-1}≡1(\mod m_i)$
  - $M_i*M_i^{-1}≡1(\mod m_i)$
  - 由于$M_i$和$m_i$必然互质，且$m_i$是质数，所以由费马定理可知，
  - $M_i^{m_1-1}≡1(\mod m_i)$
