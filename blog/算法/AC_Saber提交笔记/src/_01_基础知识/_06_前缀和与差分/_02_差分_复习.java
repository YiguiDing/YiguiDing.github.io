...
public class _02_差分_复习 {
	...
	static class DifferArray{
		...
		int[] toArr() {
			int[] arr = new int[differ.length-1];
			for(int i=1;i<arr.length;i++) {
				arr[i]=arr[i-1]+differ[i];
			}
			return arr;
		}
	}
	public static void main(String[] args) {
		...
		DifferArray differArray = new DifferArray();
		differArray.load(arr);
		while (M--!=0) {
			l=nextInt();r=nextInt();w=nextInt();
			differArray.add(l, r, w);
		}
		int[] res = differArray.toArr();
