...
import { players } from "./SelectorScene";
...
export class GameScene implements Scene {
  ...
  onUpdate(dt_ms: number): void {
    console.log("游戏 更新");
    this.players.forEach((item) => item.onGravity(this.gravity, dt_ms)); // 重力
    this.players.forEach((item) => item.onColision(this.plantforms)); // 碰撞检测
    this.players.forEach((item) => item.onUpdate(dt_ms)); // 更新
