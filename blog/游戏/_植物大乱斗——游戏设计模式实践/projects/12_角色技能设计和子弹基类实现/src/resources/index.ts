...
async function loadResources() {
  ...
  async function loadImages() {
    for (let index = 0; index < resource_descriptions.images.length; index++) {
      ...
      resource.images.set(image.name + "_right", await loadImage(image.path));
      resource.images.set(image.name + "_left", flipImage(await loadImage(image.path)));
    }
  ...
  async function loadAtlas() {
    for (let index = 0; index < resource_descriptions.atlas.length; index++) {
      ...
      resource.atlas.set(atlas.name + "_right", await Atlas.load(atlas.paths));
      resource.atlas.set(atlas.name + "_left", await Atlas.loadFlipAtlas(atlas.paths));
    }
