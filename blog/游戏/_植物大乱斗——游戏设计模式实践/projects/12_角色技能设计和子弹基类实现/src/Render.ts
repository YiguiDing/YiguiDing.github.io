...
export class Render {
  ...
  drawLine(camera: Camera, line: Line) {
    const camera_pos = camera.getPosition();
  ...
  drawCycle(camera: Camera, cycle: Cycle) {
    const camera_pos = camera.getPosition();
    this.context.beginPath();
    this.context.arc(cycle.center.x - camera_pos.x, cycle.center.y - camera_pos.y, cycle.radius, 0, 2 * Math.PI);
    this.context.stroke();
  }
}
