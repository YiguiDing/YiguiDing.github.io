...
def loss_fun(y_real,y_hat):
    return 1/2*(y_real-y_hat)**2

def D_loss_fun_Dy_hat(y_real,y_hat):
    return -y_real+y_hat

def likeHood(y_hat):
    return numpy.exp(-(loss_fun(0,y_hat)))

y_real=0
y_hat=numpy.arange(-5,5,0.1)
plt.plot(y_hat,loss_fun(y_real,y_hat))

y_real=0
y_hat=numpy.arange(-5,5,0.1)
plt.plot(y_hat,D_loss_fun_Dy_hat(y_real,y_hat))


y_hat=numpy.arange(-5,5,0.1)


plt.plot(y_hat,10 * likeHood(y_hat))
...
```
    ...
    [<matplotlib.lines.Line2D at 0x1c7302e6a60>]




    
![png](/李沫《动手学深度学习》学习笔记/3.线性神经网络/3.4softmax回归/output_2_1.png)
